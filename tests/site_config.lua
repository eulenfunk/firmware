...
-----------------------------------------------------------------------------
-- Module declaration Quelle: https://github.com/Egor-Skriptunoff/json4lua/tree/master
-----------------------------------------------------------------------------
local json = {}
...
-- Funktion um eine table in einen printbaren String zu dumpen
-- function dump(o)
...
local func, err = load(coroutine.wrap(loader))
if func then
   local dataTable = func()
   local jsonStr = json.encode(dataTable)
   print(jsonStr)
   -- local dumpJson = dump(dataTable)
   -- print(dumpJson)
else
